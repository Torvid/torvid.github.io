...
REM look into -fsanitize=address for debugging difficult crashes
...
IF "%dll_only%"=="false" (
	REM echo Compiling win32.cpp into win32.exe >> win32_output.txt
	%Compiler% %CompilerParams% win32.cpp /link %CommonLinkerFlags% quote.lib >> win32_output.txt
	REM 
	REM "C:\Program Files\LLVM\bin\clang" -o win32.exe win32.cpp -Wl,/MT -Wl,user32.lib -Wl,Gdi32.lib -Wl,Winmm.lib -Wl,Dxva2.lib
)
...
REM  -g
	...
	"C:\Program Files\LLVM\bin\clang" -Wno-everything -march=native -Ofast -gcodeview -shared -o quote.dll quote.cpp -Wl,win32.lib
	REM "C:\Program Files\LLVM\bin\clang" -g -O0 -gcodeview -Wno-everything -march=native -gcodeview  -shared -o quote.dll quote.cpp
	REM "C:\Program Files\LLVM\bin\clang" -g -O0 -Wno-everything -march=native -gcodeview -shared -o quote.dll quote.cpp
	
) ELSE (
REM /O2
	echo Compiling quote.cpp into quote.dll >> quote_output.txt
	%Compiler% %CompilerParams% /LD quote.cpp /link /PDB:quote_%RANDOM%.pdb -EXPORT:GameUpdateAndRender win32.lib >> quote_output.txt
)
