...
void DrawThreadedBox(int thread, game_memory* Memory, game_offscreen_buffer* VideoBuffer, game_sound_output_buffer* SoundBuffer)
{
    ...
    for (int i = 0; i < gameState->NextRenderCommand; i++)
    {
        ...
        {
            DrawObject(gameState, VideoBuffer, saveState->Camera, command.t, true, thread, regionCount);
        }
        if (command.Type == RenderCommandType_Box)
        {
            DrawObject(gameState, VideoBuffer, saveState->Camera, command.t, false, thread, regionCount);
        }
    ...
    MinY = ChangedSize.x;
    MaxY = ChangedSize.y;
    if (!saveState->GameViewEnabled && saveState->ThreadedRendering)
    {
        DrawRectangle(VideoBuffer, float2(MinX, MinY + 1), float2(VideoBuffer->Width, MaxY - MinY - 1), frac(float3(thread / 3.0f, thread / 6.0f, thread / 12.0f)), 0.5f);
...
extern "C" GAME_UPDATE_AND_RENDER(GameUpdateAndRender)
{
    ...
    //DrawSkybox(gameState, VideoBuffer, &saveState->Camera, -1);
    ...
    int uiOffset2 = 0;
    //DrawObject(gameState, VideoBuffer, saveState->Camera, gameState->Bodies3D[0].t, false, -1);
    //DrawObject(gameState, VideoBuffer, saveState->Camera, gameState->Bodies3D[1].t, false, -1);
    //DrawObject(gameState, VideoBuffer, saveState->Camera, gameState->Bodies3D[2].t, false, -1);
    //DrawObject(gameState, VideoBuffer, saveState->Camera, gameState->Bodies3D[4].t, false, -1);
    //DrawObject(gameState, VideoBuffer, saveState->Camera, transform(float3(4, 4, 4), BoxForward3D, BoxUp3D, float3(2, 1, 1)), true, -1);
    ...
    if (init || Input->LDown)
    {
        ...
        {
            gameState->RenderThreadSize[i] = 1.0f / saveState->ScreenThreads;
    ...
    //Memory->PlatformPrint(Memory->ThreadsExecuting[4] ? "True" : "False");

    //char aa[100] = {};
    //Append(aa, (int)(&DrawThreadedBox));
    //Memory->PlatformPrint(aa);
    // Consume draw queue
    if (saveState->ThreadedRendering)
    {
        ...
        while (true)
        {
            ...
            if (!AnyExecuting)
                break;
            if (count > 4000000) // uh oh. something has gone wrong and we didn't exit in a reasonable ammount of time.
            {
                Memory->PlatformPrint("uh oh");
                break;
            }
        }
        Memory->PlatformPrint("Pre Draw");
        ...
        DrawRectangle(VideoBuffer, 200-5, 200-5, 700, saveState->ScreenThreads*5+10, 0.0, 0.0, 0.0, 0.5);

        for (int i = 0; i < saveState->ScreenThreads; i++)
        {
            float2 start = float2(200, 200+ i * 5);
            DrawLine(VideoBuffer, start, start + float2(gameState->RenderThreadTimes[i]/5000, 0));
        }
    ...
    {
        DrawThreadedBox(-1, Memory, VideoBuffer, SoundBuffer);
    }
    ...
    //    if (gameState->step == 1)
    //    {
    //        Memory->PlatformPrint("Prepare");
    //
    //    }
    //    else if (gameState->step == 2)
    //    {
    //        Memory->PlatformPrint("Skybox");
    //        DrawSkybox(gameState, VideoBuffer, &saveState->Camera);
    //    }
    //    else if (gameState->step == 3)
    //    {
    //        Memory->PlatformPrint("Thread Start");
    //        for (int i = 0; i < 9; i++)
    //        {
    //            Memory->PlatformThreadCall(&DrawThreadedBox, i);
    //        }
    ...
    //for (int i = 0; i < saveState->ScreenThreads; i++)
    //{
    //    DrawThing(gameState->fontSpritesheet, VideoBuffer, &uiOffset2, Memory->ThreadsExecuting[i] ? "True" : "False", 0, 300);
    //}

    uiOffset2 = 0;
    for (int i = 0; i < saveState->ScreenThreads; i++)
    {
        DrawThing(gameState->fontSpritesheet, VideoBuffer, &uiOffset2, Memory->ThreadsExecuting[i] ? "True" : "False", 0, 400);
    }
    ...
    for (int i = 0; i < ArrayCount(gameState->Bodies3D); i++)
    {
        ...
        // Draw
        DrawObject(gameState, VideoBuffer, saveState->Camera, Body->t, Body->type == bodytype_sphere);
        //DrawObject(gameState, VideoBuffer, saveState->Camera, Body->t, Body->type == bodytype_sphere);
    ...
    */
    ...
    saveState->ThreadedRendering         = Toggle(gameState, Input, VideoBuffer, saveState->ThreadedRendering, "Threaded rendering", float2(10, 200));
    saveState->ThreadedRendering_Dynamic = Toggle(gameState, Input, VideoBuffer, saveState->ThreadedRendering_Dynamic, "Dynamic render regions", float2(10, 220));
    char text[100];
    for (int i = 0; i < 100; i++)
    {
        text[i] = 0;
    ...
    if (Button(gameState, Input, VideoBuffer, text, float2(10, 250)))
    {
    ...
    DrawThing(gameState->fontSpritesheet, VideoBuffer, &uiOffset, "step: ", gameState->step);
    ...
    //DrawThing(gameState->fontSpritesheet, VideoBuffer, &uiOffset, "   value: ", saveState->Camera.forward);
    ...
    */
    ...
    {
        // Editor
        //float2 MenuPlacement = float2(700, 10);
        //for (int i = 0; i < 3; i++)
        //{
        //    char SaveText[100] = "Save Level ";
        //    Append(SaveText, i);
        //    if (Button(gameState, Input, VideoBuffer, SaveText, MenuPlacement + float2(0, i * 30)))
        //    {
        //        Memory->PlatformPrint("Save");
        //    }
        //
        //    char LoadText[100] = "Load Level ";
        //    Append(LoadText, i);
        //    if (Button(gameState, Input, VideoBuffer, LoadText, float2(120, 0) + MenuPlacement + float2(0, i * 30)))
        //    {
        //        Memory->PlatformPrint("Load");
        //    }
        //}
        //
        //char* MenuText = "Play";
        ...
        //    MenuText = "Stop";
        //
        //if (Button(gameState, Input, VideoBuffer, MenuText, MenuPlacement + float2(0, 100)))
        //{
        //    gameState->playing = !gameState->playing;
        //    Memory->PlatformPrint(MenuText);
        //}
