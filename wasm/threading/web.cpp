...
void KeyChanged(int VKCode, bool IsDown)
{
    ...
    game_input* Input                       = ((game_input*)Memory                  + 1024 * 1);
...
const int GameMemory_Size = Megabytes(24);
...
void PlatformThreadCall(ThreadCallType function, int ThreadIndex)
{
    char* Memory = (char*)(&__heap_base);
    game_input* Input                       = ((game_input*)Memory                  + 1024 * 1);
    game_offscreen_buffer* Buffer           = ((game_offscreen_buffer*)Memory       + 1024 * 2);
    game_sound_output_buffer* SoundBuffer   = ((game_sound_output_buffer*)Memory    + 1024 * 3);
    game_memory* GameMemory                 = ((game_memory*)Memory                 + 1024 * 4);
    int* ThreadFunctions                    = ((int*)Memory                         + 1024 * 5);
    
    //print_num(ThreadIndex);
    if (ThreadIndex == 0)
        GameMemory->PlatformPrint("MainThreadCall 0");
    if (ThreadIndex == 1)
        GameMemory->PlatformPrint("MainThreadCall 1");
    if (ThreadIndex == 2)
        GameMemory->PlatformPrint("MainThreadCall 2");
    if (ThreadIndex == 3)
        GameMemory->PlatformPrint("MainThreadCall 3");
...
void WebThreadCall(int thread)
{
    ...
    if (GameMemory->ThreadsExecuting[thread] && GameMemory->ThreadsGo)
    {
...
void Update(int width, int height, float time)
{
    ...
    GameMemory->PermanentStorage = ((uint8*)Memory+1024*16);
    ...
    //SoundBuffer = {};
    SoundBuffer->SamplesPerSecond = 10;
