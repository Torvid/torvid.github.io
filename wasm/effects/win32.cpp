...
void PlatformReadImageIntoMemory(void* Destination, const char* File)
{
...
void PlatformReadSoundIntoMemory(void* Destination, const char* File)
{	
	read_file_result file = PlatformReadFile(0, File);
	...
	PlatformFreeFileMemory(0, file);
	
	//return (int16*)Destination;
}
...
global_variable DWORD dwThreadIdArray[100];
global_variable HANDLE hThreadArray[100];
global_variable int threadFunctionPointers[100];
...
)
{
    ...
    //HMONITOR MyMonitor = MonitorFromWindow(Window, MONITOR_DEFAULTTONULL);

    DWORD pdwMonitorCapabilities;
    DWORD pdwSupportedColorTemperatures;
    bool success = GetMonitorCapabilities(Monitor, &pdwMonitorCapabilities, &pdwSupportedColorTemperatures);

    DWORD pdwMinimumBrightness;
    DWORD pdwCurrentBrightness;
    DWORD pdwMaximumBrightness;

    if ((pdwMonitorCapabilities & MC_CAPS_BRIGHTNESS) == MC_CAPS_BRIGHTNESS)
    {
    }

    GetMonitorBrightness(Monitor, &pdwMinimumBrightness, &pdwCurrentBrightness, &pdwMaximumBrightness);
...
typedef struct MyData {
    volatile ThreadCallType* function;
...
DWORD WINAPI MyThreadFunction(LPVOID lpParam)
{
    ...
    volatile int ThreadIndex = data->ThreadIndex;
    // function inside thread
    while (true)
    {
        GameMemory.ThreadsExecuting[ThreadIndex] = true;
        ThreadCallType* functionPointer = ThreadData[ThreadIndex].function;

        functionPointer(ThreadIndex, &GameMemory, &Buffer, &SoundBuffer);

        GameMemory.ThreadsExecuting[ThreadIndex] = false;
...
void PlatformThreadCall(ThreadCallType function, int ThreadIndex)
{
    ...
    //DWORD result = WaitForSingleObject(hThreadArray[ThreadIndex], 0);
    //ThreadData[ThreadIndex].function = function;
    //ThreadData[ThreadIndex].ThreadIndex = ThreadIndex;
    //if (result == WAIT_OBJECT_0 || hThreadArray[ThreadIndex] == NULL) {
...
#define GameUpdateHz 60
    ...
    GameMemory.PermanentStorageSize = Kilobytes(1); // Stored in memory and copied to disk at EOF
    GameMemory.TransientStorageSize = Megabytes(16); // Stored in live memory
    ...
    data = PlatformReadFile(0, "data.bin");
   ...
   // }
    ...
    while (GlobalRunning)
    {
        ...
        if (SaveDelay == 0)
        {
            ...
            read_file_result data = {};
            data.Contents = GameMemory.PermanentStorage;
            data.ContentsSize = GameMemory.PermanentStorageSize;
            PlatformWriteFile(0, "data.bin", data);
        }
