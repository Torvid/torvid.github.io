...
void QueueDrawEffect(GameState* gameState, transform t, Image* texture1, Image* texture2, int ShaderIndex, EffectData effect, float3 Color)
{
    gameState->RenderCommands[gameState->NextRenderCommand].effect = effect;
    gameState->RenderCommands[gameState->NextRenderCommand].t = t;
    gameState->RenderCommands[gameState->NextRenderCommand].Type = RenderCommandType_Effect;
    gameState->RenderCommands[gameState->NextRenderCommand].texture1 = texture1;
    gameState->RenderCommands[gameState->NextRenderCommand].texture2 = texture2;
    gameState->RenderCommands[gameState->NextRenderCommand].ShaderIndex = ShaderIndex;
...
void DrawImageScaled(GameState* gameState, game_offscreen_buffer* Buffer, Image* image, float2 Pos, float2 Size, float3 Color, int Shader = 0, int Region = -1, int Regions = 9, uint8 depth = 0)
{
    if (image == 0)
    {
    ...
    int2 ChangedSize = GetThreadRegion(gameState, Regions, Region, Buffer->Height, MinY, MaxY);
    MinY = ChangedSize.x;
    MaxY = ChangedSize.y;
...
uint32 Shader(GameState* gameState, game_offscreen_buffer* VideoBuffer, float3 LocalPos, float depth, float3 Normal, float3 NormalLocal, float3 ViewDirection, bool Sphere, Image* texture1, Image* texture2, int ShaderIndex, float3 scale, bool selected, bool hovered)
{
    ...
    if (ShaderIndex == 0)
        SurfaceColor = SurfaceShaderExample(VideoBuffer, texture1, texture2, float3(1, 1, 1), LocalPos, depth, Normal, NormalLocal, ViewDirection, Sphere ? EntityType_Sphere : EntityType_Box, scale);
    else if (ShaderIndex == 1)
        SurfaceColor = SurfaceShaderSimple(VideoBuffer, texture1, texture2, float3(1, 1, 1), LocalPos, depth, Normal, NormalLocal, ViewDirection, Sphere ? EntityType_Sphere : EntityType_Box, scale);
    else if (ShaderIndex == 2)
        SurfaceColor = SurfaceShaderTexture(VideoBuffer, texture1, texture2, float3(1, 1, 1), LocalPos, depth, Normal, NormalLocal, ViewDirection, Sphere ? EntityType_Sphere : EntityType_Box, scale);
    else if (ShaderIndex == 3)
        SurfaceColor = SurfaceShaderTextureTop(VideoBuffer, texture1, texture2, float3(1, 1, 1), LocalPos, depth, Normal, NormalLocal, ViewDirection, Sphere ? EntityType_Sphere : EntityType_Box, scale);
...
void DrawEffect(GameState* gameState, game_offscreen_buffer* VideoBuffer, transform Camera, RenderCommand* command, int Region = -1, int Regions = 9, int DrawIndex = 0)
{
    float AspectRatio = 0.5625f;
    float3 Up = Camera.up * AspectRatio;
    transform CameraTransform = Camera;
    ...
    float Time = gameState->GameTime - effect->startTime;
    ...
    for (int i = 0; i < MaxParticles; i++)
    {
        ...
        float size = (100 * lerp(effect->startSize, effect->endSize, tNormalized)) / CenterPoint.z;
        float2 Pos = float2(CenterPoint.x, CenterPoint.y) - float2(size, size) * 0.5;
        float2 Size = float2(size, size);

        DrawImageScaled(gameState, VideoBuffer, command->texture1, Pos, Size, command->color, command->ShaderIndex, Region, Regions, particleDepth);
    }
}
...
void Text(GameState* gameState, game_offscreen_buffer* VideoBuffer, float2* Pos, const char* text, int value)
{
    char String[100] = {};
    Append(String, text);
    Append(String, value);
...
void DrawThreadedBox(int thread, game_memory* Memory, game_offscreen_buffer* VideoBuffer, game_sound_output_buffer* SoundBuffer)
{
    ...
    for (int i = 0; i < gameState->NextRenderCommand; i++)
    {
        ...
        {
            DrawEffect(gameState, VideoBuffer, saveState->Camera, &command, thread, regionCount);
        }
