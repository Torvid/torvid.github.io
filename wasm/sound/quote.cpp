...
internal void GameOutputSound(GameState* GameState, game_sound_output_buffer* SoundBuffer, float ToneHz, float ToneVolume)
{
    ...
    float TonePeriod = (float)SoundBuffer->SamplesPerSecond / ToneHz;
    ...
    for (int i = 0; i < SoundBuffer->SampleCount; i++)
    {
        ...
        Sample SampleValue = Sample((int16)(SineValue * ToneVolume), (int16)(SineValue * ToneVolume));
...
extern "C" GAME_UPDATE_AND_RENDER(GameUpdateAndRender)
{
    ...
    if (!Memory->Initialized) // Happens on startup and when the user presses space
    {
        ...
        InitializeArena(&gameState->soundArena,   Megabytes(12), (uint8*)Memory->TransientStorage + sizeof(GameState) + Megabytes(1) + Megabytes(12));
        ...
        //game_memory* Memory, GameState* gameState,
    ...
    float2 MainUIPos = float2(0, 0);
    Text(gameState, VideoBuffer, &MainUIPos, "Controls");
    Text(gameState, VideoBuffer, &MainUIPos, "  Space to reset program.");
    //Text(gameState, VideoBuffer, &MainUIPos, "  Left click to control string synth.");
    ...
    if (!saveState->GameViewEnabled && false)
    {
