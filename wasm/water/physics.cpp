...
for (int i = 0; i < ArrayCapacity(engineState->Bodies2D); i++)
{
    ...
    for (int j = 0; j < 10; j++)
    {
        for (int k = 0; k < ArrayCapacity(engineState->Bodies2D); k++)
        {
...
void UpdatePhysics(GameMemory* memory, EngineState* engineState, EngineSaveState* engineSaveState, VideoBuffer* videoBuffer)
{
    for (int i = 0; i < ArrayCapacity(engineState->entities); i++)
    {
        ...
        for (int j = 0; j < 10; j++)
        {
            for (int k = 0; k < ArrayCapacity(engineState->entities); k++)
            {
                ...
                if (OtherBody->type == EntityType_Box)
                    DeIntersect = SphereBoxDeIntersection(OtherBody->transform, Body->transform);
                else if (OtherBody->type == EntityType_Sphere)
                    DeIntersect = SphereSphereDeIntersection(OtherBody->transform.position, OtherBody->transform.scale.x, Body->transform.position, Body->transform.scale.y);
                ...
                if (length(DeIntersect) > 0)
                {
                    ...
                    //float MoveDistance = distance(engineState->LastSpherePos, engineState->SpherePos);
                    // TODO: If the other object is also dynamic, do something else here.
                    Body->transform.position += DeIntersect;
                    if (!OtherBody->kinematic)
                        HitDynamicBody = OtherBody;
                }
            }
        ...
        Body->transform = rotate(Body->transform, Body->angularVelocityAxis, -Body->angularVelocity * 200);

        DrawLine3D(videoBuffer, engineState->camera, Body->transform.position, Body->transform.position + Body->velocity);
    }
}