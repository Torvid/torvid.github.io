...
void DrawSoundBuffer(VideoBuffer* videoBuffer, game_sound_output_buffer* SoundBuffer, float3 Color)
{
    int LastX = 0;
    int LastY = 0;
...
void DrawSoundRaw(VideoBuffer* videoBuffer, Sound* sound, float2 pos, int StartSample, int EndSample)
{
...
void DrawSound(VideoBuffer* videoBuffer, Sound* sound, float2 pos, float2 size, int currentSample = 0)
{
...
Sound* LoadSound(EngineState* engineState, MemoryArena* arena, const char* path, float Length)
{
    ...
    Sound* newSound = PushStruct(arena, Sound);
    StringCopy(path, newSound->name);
    Sample* SoundLocation = PushArray(arena, sampleCount + padding, Sample);
    ...
    //ArrayAdd(engineState->sounds, newSound);
    // Check values
    //Sample a[100];
    //for (int i = 0; i < 100; i++)
    //{
...
{
    for (int i = 0; i < ArrayCapacity(engineState->sounds); i++)
    {
...
void drawString(VideoBuffer* videoBuffer, float* Points, int Length, int PixelsPerSample, int offsetx = 0, int offsety = 0, float3 Color = {})
{
...
void RenderFFT(VideoBuffer* videoBuffer, game_sound_output_buffer* SoundBuffer, GameInput* Input, string_data* StringData, float2 Pos, float3 Color)
{
    if (SoundBuffer->SampleCount > 0)
    {
        ...
        for (int y = 0; y < 100; y++)
        {
            ...
            for (int32 i = 0; i < SoundBuffer->SampleCount; i++)
            {
                float perspective = windingfrequency * (float)i;
                float vecX = sin(perspective);
                float vecY = cos(perspective);
...
void RenderStringSoundLine(VideoBuffer* videoBuffer, game_sound_output_buffer* SoundBuffer, GameInput* Input, string_data* StringData, float2 Pos, float3 Color)
{
    drawString(videoBuffer, StringData->Pos, StringData->Segments, StringData->PixelsPerSample, Pos.x, Pos.y, Color);
}

void StringSound(VideoBuffer* videoBuffer, game_sound_output_buffer* SoundBuffer, GameInput* Input, string_data* StringData, bool MouseIsOverUI, float2 Pos)
{
