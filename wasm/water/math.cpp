...
#if WIN32
    // Windows, use math.h like a normal person, also enables intrinsics for more speeeeeed
    #define MATH_TYPE PLATFORM_MATH
#elif WASM
    // Webassembly.
    #define MATH_TYPE C_MATH // safest bet
#elif ESP32
...
float3  clamp(float3 a, float  minimum, float  maximim) { return min(max(a, float3(minimum, minimum, minimum)), float3(maximim, maximim, maximim)); }
...
float2 inverselerp(float2 a, float2 b, float v) { return (float2(v, v) - a) / (b - a); }
float3 inverselerp(float3 a, float3 b, float v) { return (float3(v, v, v) - a) / (b - a); }
...
// These functions are defined in math.h on the ESP32, so we don't need to define them here.
...
float mod(float a, int b) { return frac(a / (float)b) * (float)b; }
float mod(int a, float b) { return frac((float)a / b) * b; }
...
float average(float2 Input) { return (Input.x + Input.y) * 0.5; }
float average(float3 Input) { return (Input.x + Input.y + Input.z) * (1.0f/3.0f); }
...
float3 normalizeFast(float3 a)
{
    return a * _mm_cvtss_f32(_mm_rsqrt_ps(_mm_set1_ps(dot(a, a))));
}
#else
float3 normalizeFast(float3 a)
{
    return a / sqrtFast(dot(a, a));
}
#endif
...
float2 rotate(float2 v, float perspective)
{
    float x = v.x * cos(perspective) - v.y * sin(perspective);
    float y = v.x * sin(perspective) + v.y * cos(perspective);
    return float2(x, y);
}
...
{
    return float2(pow(a.x, b.x), pow(a.y, b.y));
}
...
// sin with a 0 - 1 range instead of 0 to tau range
float sinTurns(float x)
{
    return sin(x * 3.14152128 * 2.0);
}
// cos with a 0 - 1 range instead of 0 to tau range
float cosTurns(float x)
{
    return cos(x * 3.14152128 * 2.0);
}
...
Transform rotate(Transform t, float3 Axis, float perspective)
{
    t.forward = RotateAroundAxis(t.forward, Axis, perspective);
    t.up = RotateAroundAxis(t.up, Axis, perspective);
...
// X and Y are UVs, Z is face index
float2 RayCubemap_out(float3 v, int* index)
{
    float3 vAbs = abs(v);
    float3 habs = 0.5f / vAbs;
    if (vAbs.z >= vAbs.x && vAbs.z >= vAbs.y)
